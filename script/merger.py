...
class merge():
    ...
    def get_variables(self, variable_root):
        ...
        self.variables = varibale_dict
        self.variables_hash = varibale_hash_dict
    ...
    def merged_dic(self):
        ...
        for file, records in self.section.items():
            ...
            variable_ann = self.variables.get(file)
        ...
        self.section_variable = file_section_variavle
    ...
    def diagnostic_filterring(self):
        ...
        for file, sections in self.section_variable.items():
                        ...
                        if section not in self.required_headers:
                                if (record["label"].split("_SUG_")[-1] in self.required_main_variables or
                                 record["label"].split("_SUG_")[-1] in self.required_second_variables):
                                    self.section_variable[file][section].remove(record)
                                    counter += 1
    ...
    def save_accepted_varibales(self, ann_final):
        ...
        for file, section_varibale in self.section_variable.items():
            T = 0
                ...
                for var in variables:
                    if var["T"].startswith("T"):
                        ...
                        final_brat_f.write('T' + str(T) +
                                           ...
                                           "\t" + var['text'] + "\n")
                        if var['T'].replace("T", "#") in self.variables_hash[file].keys():
                            final_brat_f.write('#' + str(T) + "\t" + self.variables_hash[file][var['T']
